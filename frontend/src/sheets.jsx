...
import { fmtDate, fmtNum, fmtVol, fmtDur, durPart, todayISO, uid, exCount, DAYN, MONTHS_LONG, ACCENTS, setCount } from './lib/format.js'
...
import { t, instrFor, loadBaseInstructions, exerciseNameFor, getLang, INSTR_LANGS, instrPackFor } from './lib/i18n.js'
      ...
      <div className="list" style={{ gap: 0 }}>
        {recent.map(b => <div key={b.d} className="row between" style={{ padding: '9px 2px', borderBottom: '1px solid var(--sep)' }}>
          ...
          <span className="row" style={{ gap: 12 }}><b>{fmtNum(b.w)} {unit}</b>
            <button className="iconbtn" style={{ width: 32, height: 30, borderRadius: 8, fontSize: 15, color: 'var(--red-ink)' }} onClick={() => delEntry(b.d)} aria-label={t('Delete')}><Icon name="trash" /></button></span>
        </div>)}
      </div>
    </>}
  </>
...
function ExerciseDetail({ ex, close }) {
  ...
  // Instruksi bahasa Inggris dimuat saat sheet ini dibuka, bukan di muat pertama app: 626 KB
  // yang cuma dibaca di sini, sementara katalognya dibutuhkan di setiap layar. Sisa sheet-nya
  // tampil seketika; langkah-langkahnya menyusul beberapa ratus milidetik kemudian.
  const [steps, setSteps] = useState(() => instrFor(ex))
  useEffect(() => {
    let alive = true
    loadBaseInstructions().then(() => { if (alive) setSteps(instrFor(ex)) })
    return () => { alive = false }
  }, [ex])
  return <>
    ...
    {steps.length > 0 &&<><h4 className="sec">{t('How to')}{!instrPackFor(getLang()) && <span className="dim" style={{ textTransform: 'none', letterSpacing: 0 }}> · {t('instructions in English')}</span>}</h4><ol className="steps-list">{steps.map((s, i) => <li key={i}>{s}</li>)}</ol></>}
  </>
}
...
function PlanTools({ close }) {
  ...
  return <>
    ...
    <div className="dim small" style={{ margin: '7px 2px 0', lineHeight: 1.4 }}>{t('A small file a friend imports into their own Halal Pro Gym — routines only, none of your workouts or weigh-ins.')}</div>
    {!MOBILE && <>
      <div style={{ height: 12 }} />
...
function PlanImport({ bundle, close }) {
  ...
  return <>
    ...
    {bundle.scheduledDays > 0 && <div className="row between" style={{ padding: '10px 2px', borderTop: '1px solid var(--sep)', borderBottom: '1px solid var(--sep)', marginBottom: 16, gap: 12 }}>
      <div><div className="tt" style={{ fontSize: 15 }}>{t('Use this weekly schedule')}</div><div className="small dim">{t('Replaces your current Sunday–Saturday assignments.')}</div></div>
      <Switch checked={schedule} onChange={setSchedule} label={t('Use this weekly schedule')} />
    </div>}
...
function FinishSummary({ w, prs, e1prs = [], close }) {
  ...
  return <div style={{ textAlign: 'center', padding: '8px 0' }}>
    ...
    <div className="tiles" style={{ textAlign: 'left' }}>
      ...
      <div className="tile"><div className="l">{t('Sets')}</div><div className="v" style={{ fontSize: '1.1rem' }}>{t(setsDone(w) === 1 ? '{0} set · {1} work' : '{0} sets · {1} work', setsDone(w), workSetsDone(w))}</div></div>
