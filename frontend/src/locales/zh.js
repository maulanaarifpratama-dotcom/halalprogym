...
export default {
  ...
  "This browser doesn't support passkeys — you can still use Halal Pro Gym locally on this device.": '此浏览器不支持通行密钥——你仍可在本设备上本地使用 Halal Pro Gym。',
  ...
  'Rest-timer alerts, even if Halal Pro Gym is closed.': '休息计时提醒，即使 Halal Pro Gym 已关闭。',
  ...
  'Your server': '你的服务器',
  'Synced with your Halal Pro Gym server.': '已与你的 Halal Pro Gym 服务器同步。',
  'Disconnect': '断开连接',
  ...
  'Sync this device to your own self-hosted Halal Pro Gym instead.': '改为将此设备与你自己的自托管 Halal Pro Gym 同步。',
  'Pair the mobile app': '配对手机应用',
  'Connect the Halal Pro Gym app on your phone to this account.': '将手机上的 Halal Pro Gym 应用连接到此账户。',
  'Could not generate a code': '无法生成配对码',
  'On the Halal Pro Gym app, choose “Connect to my server”, then enter this address and the code below. It expires in 5 minutes.': '在 Halal Pro Gym 应用中选择“连接到我的服务器”，然后输入此地址和下面的配对码。5 分钟后失效。',
  'Enter your server address and the code': '输入你的服务器地址和配对码',
  'Connected': '已连接',
  'Could not connect': '无法连接',
  'Open Settings → “Pair the mobile app” on the Halal Pro Gym site you’re already signed into, then enter its address and the code shown there.': '在你已登录的 Halal Pro Gym 网站上，打开设置 →“配对手机应用”，然后输入其地址和那里显示的配对码。',
  'Server address (e.g. gym.example.com)': '服务器地址（例如 gym.example.com）',
  'Pairing code': '配对码',
  'Connecting…': '连接中…',
  'Connect': '连接',
  'How do you want to use Halal Pro Gym?': '你想如何使用 Halal Pro Gym？',
  'Use on this device': '在此设备上使用',
  'Local keeps everything on this phone. Connecting syncs to your own Halal Pro Gym server instead — you can switch later in Settings.': '本地模式会把所有内容保留在这部手机上。连接则会改为与你自己的 Halal Pro Gym 服务器同步——之后可以在设置中更改。',
